...
A `Behavior` itself is just a `PartialFunction` from `State[A] => Actions[A]` where A is the aggregate type (the Lottery in this example), `State` is whether `Unitialized` or `Initialized` and `Actions` is the set of **Command Handlers** and **Event Handlers** that are applicable for a given `State`.
...
<2> The `AkkaBackend` will start its own `ActorSystem`, but of course you can use your own `ActorSystem` by overriding `AkkaBackend.actorSystem`. For instance, if you run a Play application you'll probably want to use Play's ActorSystem.
