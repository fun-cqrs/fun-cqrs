...
**Fun.CQRS** is a Scala library for building CQRS/ES application. It provides the basic blocks to build event driven aggregates backed by **Akka** for Event Sourcing.
...
- An user can remove participant +
...
We make a distinction between `Commands` and `Events` that instantiate an **Aggregate** (creational `Commands` and `Events`) and those that must be validated against an existing **Aggregate** instance (update `Commands` and `Events`). This is done by defining the `Bindings` inside `whenCreating` and `whenUpdating` blocks.
