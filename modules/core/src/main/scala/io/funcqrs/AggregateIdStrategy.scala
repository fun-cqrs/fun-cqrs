...
sealed trait AggregateIdStrategy[A <: AggregateLike] extends AggregateAliases {
  ...
  /** Partial function to transform incoming messages to CommandMsg */
  type BeforeReceiveCreateCommand = PartialFunction[Any, CommandMsg]
...
case class CommandMsg(id: AggregateId, cmd: DomainCommand)
/**
 ...
 * @tparam A the Aggregate type
 */
class AssignedIdStrategy[A <: AggregateLike] extends AggregateIdStrategy[A] with AggregateMessageExtractors {

  def beforeReceiveCreateCommand: BeforeReceiveCreateCommand = {
    case cmdMsg: CommandMsg => cmdMsg
    case (GoodId(id), cmd: Command) => CommandMsg(id, cmd)
  }
...
trait GeneratedIdStrategy[A <: AggregateLike] extends ManagedIdStrategy[A] {
  ...
  /** Receives Command and returns CommandMsg(Id, Command), where Id is generated by generatedId() methods */
  def beforeReceiveCreateCommand: BeforeReceiveCreateCommand = {
    case cmd: Command => CommandMsg(generateId(), cmd)
  }
...
trait SingletonIdStrategy[A <: AggregateLike] extends ManagedIdStrategy[A] {
  ...
  /** Receives a Command and returns CommandMsg(Id, Command), where Id is fixed */
  def beforeReceiveCreateCommand: BeforeReceiveCreateCommand = {
    case cmd: Command => CommandMsg(id, cmd)
  }
