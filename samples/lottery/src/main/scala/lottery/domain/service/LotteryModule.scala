...
trait LotteryModule extends AkkaModule {
  ...
  // funCQRS.aggregate[AggregateType]
  //        .behavior(id => AggregateType.behavior(id))
  //        .withAutoGeneratedId(() => AggregateTypeId())
  //
  // funCQRS.projection(new LotteryViewProjection)
  //        .identifiedBy("LotteryViewProjectionActor")
  //        .withSource(new SourceProvider)
  //        .offsetPersistenceStrategy(OffsetNotPersisted)
